...
export class Laboratory {
  ...
  classique(): void {
    console.log(
      'je suis une fonction classique qui ne prend aucun argument et renvoi rien'
    );
  }
  somme(x: number, y: number): number {
    //prend 2 arguments et renvoi la somme
    return x + y;
  }
  ...
  tests(): void {
    ...
    const data$ = new Observable((sub) => {
      // on définit ici un observable (qui peut être observé)
    ...
    let counter = 0;
    const observer1 = data$.subscribe({
      //l'observateur1 souscrit à l'observable et gère 2 cas :
